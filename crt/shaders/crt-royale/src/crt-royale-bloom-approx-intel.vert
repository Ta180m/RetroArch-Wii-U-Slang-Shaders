...
void main()
{
   gl_Position = global . MVP * Position;
        vec2 vTexCoord = TexCoord;
               vec2 video_uv = vTexCoord * params . SourceSize . xy / params . SourceSize . xy;
    tex_uv = video_uv * params . SourceSize . xy /
                                  params . SourceSize . xy;


    estimated_viewport_size_x =
         params . SourceSize . xy . y * global . geom_aspect_ratio_x / global . geom_aspect_ratio_y;
               ...
               vec2 dxdy_min_scale = params . SourceSize . xy / params . OutputSize . xy;
    texture_size_inv = vec2(1.0)/ params . SourceSize . xy;
    ...
    tex_uv_to_pixel_scale = params . OutputSize . xy *
                                  params . SourceSize . xy / params . SourceSize . xy;





               vec2 orig_video_size = params . SourceSize . xy;
          float y_step = 1.0 + float(is_interlaced(orig_video_size . y));
               vec2 il_step_multiple = vec2(1.0, y_step);

    uv_scanline_step = il_step_multiple / params . SourceSize . xy;
}
