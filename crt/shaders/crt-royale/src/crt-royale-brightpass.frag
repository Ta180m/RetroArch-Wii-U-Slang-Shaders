...
uniform sampler2D MASKED_SCANLINES;
...
void main()
{
        ...
        tex2D_linearize(MASKED_SCANLINES, scanline_tex_uv). rgb;
          ...
          float mask_amplify = get_mask_amplify();
               vec3 intensity = intensity_dim * undim_factor * mask_amplify *
                      global . levels_contrast;




               vec3 phosphor_blur_approx = global . levels_contrast * tex2D_linearize(
                          BLOOM_APPROX, blur3x3_tex_uv). rgb;



          float bloom_sigma = get_final_bloom_sigma(bloom_sigma_runtime);
          float center_weight = get_center_weight(bloom_sigma);
               vec3 max_area_contribution_approx =
        max(vec3(0.0, 0.0, 0.0), phosphor_blur_approx - center_weight * intensity);



               vec3 area_contrib_underestimate =
                                 global . bloom_underestimate_levels * max_area_contribution_approx;
               vec3 intensity_underestimate =
                                 global . bloom_underestimate_levels * intensity;
            ...
            ((vec3(1.0, 1.0, 1.0)- area_contrib_underestimate)/
            intensity_underestimate - vec3(1.0, 1.0, 1.0))/(center_weight - 1.0);

               vec3 blur_ratio = clamp(blur_ratio_temp, 0.0, 1.0);


               vec3 brightpass = intensity_dim *
                                                                   mix(blur_ratio, vec3(1.0, 1.0, 1.0), global . bloom_excess);
