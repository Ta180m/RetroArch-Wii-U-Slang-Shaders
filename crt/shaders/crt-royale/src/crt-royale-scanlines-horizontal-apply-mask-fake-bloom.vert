...
void main()
{
   gl_Position = global . MVP * Position;
    ...
    video_uv = tex_uv * params . SourceSize . xy / params . SourceSize . xy;
    scanline_texture_size_inv =
             vec2(1.0, 1.0)/ global . VERTICAL_SCANLINESSize . xy;

    scanline_tex_uv = video_uv * global . VERTICAL_SCANLINESSize . xy *
        scanline_texture_size_inv;
    blur3x3_tex_uv = video_uv * global . BLOOM_APPROXSize . xy /
                               global . BLOOM_APPROXSize . xy;
    halation_tex_uv = video_uv * global . HALATION_BLURSize . xy /
              ...
              float mask_sample_mode = get_mask_sample_mode();
                   vec2 mask_resize_texture_size = mask_sample_mode < 0.5 ?
                                  global . MASK_RESIZESize . xy : mask_texture_large_size;
                   vec2 mask_resize_video_size = mask_sample_mode < 0.5 ?
                                global . MASK_RESIZESize . xy : mask_texture_large_size;
    ...
    mask_tile_start_uv_and_size = get_mask_sampling_parameters(
        mask_resize_texture_size, mask_resize_video_size, params . OutputSize . xy,
