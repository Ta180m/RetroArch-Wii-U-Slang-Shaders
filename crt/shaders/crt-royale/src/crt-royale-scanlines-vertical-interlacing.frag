...
void main()
{
    ...
    float dist;
        ...
        texture_size_inv, il_step_multiple, FrameCount, dist);
    ...
    if(beam_misconvergence)
    {
       ...
       float sigma_range = max(global . beam_max_sigma, global . beam_min_sigma)-
                     global . beam_min_sigma;
       float shape_range = max(global . beam_max_shape, global . beam_min_shape)-
                     global . beam_min_shape;



               vec3 scanline2_contrib = scanline_contrib(dist2,
        scanline2_color, ph, sigma_range, shape_range);
               vec3 scanline3_contrib = scanline_contrib(abs(vec3(1.0, 1.0, 1.0)- dist2),
    ...
    if(beam_num_scanlines > 5.5)
    {
            ...
            scanline_contrib(dist2 + vec3(2.0, 2.0, 2.0), scanline0_color,
                ph, sigma_range, shape_range);
                   vec3 scanline1_contrib =
            scanline_contrib(dist2 + vec3(1.0, 1.0, 1.0), scanline1_color,
                ph, sigma_range, shape_range);
                   vec3 scanline4_contrib =
            scanline_contrib(abs(vec3(2.0, 2.0, 2.0)- dist2), scanline4_color,
                ph, sigma_range, shape_range);
    ...
    else if(beam_num_scanlines > 4.5)
    {
            ...
            scanline_contrib(abs(vec3(2.0, 2.0, 2.0)- dist2), scanline4_color,
                ph, sigma_range, shape_range);
                   vec3 dist0or5 =
                                                                                mix(dist2 + vec3(2.0, 2.0, 2.0), vec3(3.0, 3.0, 3.0)- dist2, dist_round);
    ...
    {
                   vec3 dist1or4 =
                                                                                mix(dist2 + vec3(1.0, 1.0, 1.0), vec3(2.0, 2.0, 2.0)- dist2, dist_round);
