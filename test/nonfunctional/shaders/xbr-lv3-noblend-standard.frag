...
void main()
{
    ...
    edr_left = vec4(lessThanEqual(2.0 * df(f, g), df(h, c)))* interp_restriction_lv2_left * edr *(not(edri . yzwx)* eq(e, c));
    edr_up = vec4(greaterThanEqual(df(f, g), 2.0 * df(h, c)))* interp_restriction_lv2_up * edr *(not(edri . wxyz)* eq(e, g));
