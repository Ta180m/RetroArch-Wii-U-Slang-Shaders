...
{
 return(2.0 *(df(c1, c2)+ df(c1, c0)+ df(e2, e1)+ df(e2, e3))+ 1.0 *(df(d2, d3)+ df(d0, d1))+ - 1.0 *(df(d1, d3)+ df(d0, d2))+ 4.0 * df(d1, d2)+ - 1.0 *(df(c0, c2)+ df(e1, e3))+ 1.0 *(df(b0, b1)+ df(f2, f3)));
}

float hv_wd(float i1, float i2, float i3, float i4, float e1, float e2, float e3, float e4)
{
 return(4.0 *(df(i1, i2)+ df(i3, i4))+ 2.0 *(df(i1, e1)+ df(i2, e2)+ df(i3, e3)+ df(i4, e4)));
}
...
void main()
{
  ...
  if(any(lessThan(fract(vTexCoord * params . SourceSize . xy / 4.0), vec2(0.5, 0.5))))
 {
 ...
 else
 {
  ...
  vec2 g1 = vec2(4.0 / params . SourceSize . x, 0.0);
  vec2 g2 = vec2(0.0, 4.0 / params . SourceSize . y);
  ...
  vec4 w1 = vec4(-(XBR_WEIGHT * 1.29633 / 10.0),(XBR_WEIGHT * 1.29633 / 10.0)+ 0.5,(XBR_WEIGHT * 1.29633 / 10.0)+ 0.5, -(XBR_WEIGHT * 1.29633 / 10.0));
  vec4 w2 = vec4(-(XBR_WEIGHT * 1.75068 / 10.0 / 2.0),(XBR_WEIGHT * 1.75068 / 10.0 / 2.0)+ 0.25,(XBR_WEIGHT * 1.75068 / 10.0 / 2.0)+ 0.25, -(XBR_WEIGHT * 1.75068 / 10.0 / 2.0));
